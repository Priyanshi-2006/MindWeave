...
type Ctx = {
  ...
  submitRound: (game: GameType, metrics: RoundMetrics, diag?: Diagnostics) => RoundResult;
...
export function ProfileProvider({ children }: { children: ReactNode }) {
  ...
  const finishAssessment = useCallback((skills: Partial<ChildProfile["skills"]>) => {
    setProfile((p) => (p ? { ...p, assessmentDone: true, skills: { ...p.skills, ...skills } } : p));
  }, []);
  ...
  const dismissBonus = useCallback(() => {
    setProfile((p) => (p ? { ...p, bonus: { ...p.bonus, dismissedOn: todayKey() } } : p));
  }, []);

  const updateSettings = useCallback((patch: Partial<ParentSettings>) => {
    setProfile((p) => (p ? { ...p, settings: { ...p.settings, ...patch } } : p));
  }, []);
