...
export function AppShell({ children }: { children: ReactNode }) {
  const { profile, accessibility, updateAccessibility } = useProfile();
  ...
  return (
    ...
    >
      ...
      <div className="bg-muted/90 border-b border-border py-2 px-4 text-xs font-semibold text-muted-foreground flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            className={cn(
              "toy-press flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-bold transition-all cursor-pointer",
              isHighContrast
                ? "bg-primary text-primary-foreground border-primary shadow-soft"
                : "bg-card border-border text-foreground hover:bg-muted"
            )}
            aria-pressed={isHighContrast}
            title={isHighContrast ? "Switch to Standard Contrast" : "Switch to Dark High Contrast Mode"}
          >
            <span>{isHighContrast ? "☀️ Standard Contrast" : "👁️ Dark High Contrast"}</span>
          </button>
          <button
            onClick={() => {
            ...
            className="toy-press flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1 text-xs font-bold text-foreground hover:bg-muted transition-colors cursor-pointer"
      ...
      <header className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-4 py-4">
        <Link to="/" className="flex items-center" aria-label="MindWeave Home">
          <img
            src={logoImg}
            alt="MindWeave"
            className="h-auto w-32 sm:w-36 md:w-40 object-contain transition-transform hover:scale-102 drop-shadow-sm"
            width={160}
            height={90}
          />
        </Link>
        <nav className="flex items-center gap-2 text-base font-bold">
          <Link
            to="/"
            className="rounded-full px-4 py-2 hover:bg-muted text-foreground transition-colors"
            activeOptions={{ exact: true }}
            activeProps={{ className: "bg-card border-2 border-border text-foreground shadow-soft" }}
          >
            Home
          </Link>
          <Link
